...
XanoScript RULE: All `where` expressions inside `db.query` must be written on a single line. A newline after `where =` or before any `&&` operator will cause parser errors such as “unexpected 'where'” or “unexpected '&&'”.
...
Sub-rules:
- `==?` is valid for ignore-if-null comparisons.
- `includes` does NOT support `?`.
- Table names in `db.query` may be quoted or unquoted.
- Never split the boolean expression across multiple lines.
